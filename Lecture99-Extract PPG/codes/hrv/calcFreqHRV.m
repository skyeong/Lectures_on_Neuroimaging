...
idx_MF  = f>=0.08  & f<0.15;   % Rollin McCraty et al (1995) Am J Cardiology.
idx_GF  = f>=0.12  & f<0.18;
...
S_MF  = sum(RR_fft(idx_MF ))*deltaFreq;    % areal power (ms)^2
S_GF  = sum(RR_fft(idx_GF ))*deltaFreq;    % areal power (ms)^2
...
STAT(1).nuMF   = 100*S_MF/(S_TOT-S_VLF);   % relative power
STAT(1).nuGF   = 100*S_GF/(S_TOT-S_VLF);   % relative power
STAT(1).nuLF   = 100*S_LF/(S_TOT-S_VLF);   % relative power
STAT(1).nuHF   = 100*S_HF/(S_TOT-S_VLF);   % relative power

STAT(1).MF     = S_MF;        % absolute power
STAT(1).GF     = S_GF;        % absolute power
